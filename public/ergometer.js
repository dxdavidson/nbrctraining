...
(function (ergometer) {
    class WaitResponseBuffer {
        ...
        processedBuffer() {
            this.removeRemainingCommands();
            this.remove();
            // status byte bits 0x30: 0x00 Ok, 0x10 Reject, 0x20 Bad, 0x30 Not ready
            if (this.prevFrameState !== 0) {
                var reason = ["Ok", "Reject", "Bad", "Not ready"][this.prevFrameState] || this.prevFrameState;
                if (this._reject)
                    this._reject(`Command rejected by monitor (${reason})`);
                if (this._monitor.logLevel >= ergometer.LogLevel.error)
                    this._monitor.handleError(`command rejected by monitor: ${reason}`);
            }
    ...
    class PerformanceMonitorBle extends ergometer.PerformanceMonitorBase {
        ...
        */
        ...
        handleCSafeNotifications() {
            ...
            // On Android the first GATT operations right after connecting can spuriously
            // fail with "GATT operation failed for unknown reason" before the link has settled.
            // Silently continuing without this notification means every future csafe send times out,
            // so retry a few times instead of swallowing the error.
            var attempt = (retriesLeft) => {
                return this.enableNotification(ergometer.ble.PMCONTROL_SERVICE, ergometer.ble.RECEIVE_FROM_PM_CHARACTERISIC, receive)
                    .catch((e) => {
                    if (retriesLeft <= 0)
                        throw e;
                    this.traceInfo(`enable notifications csafe failed, retrying: ${e}`);
                    // enableNotification marks the guid registered before the driver call resolves, undo that so the retry isn't skipped.
                    this._registeredGuids[ergometer.ble.RECEIVE_FROM_PM_CHARACTERISIC] = 0;
                    return new Promise((resolve) => setTimeout(resolve, 300)).then(() => attempt(retriesLeft - 1));
                });
            };
            return attempt(3).catch(this.getErrorHandlerFunc(""));
        }
        /**
         *
         * @param data
